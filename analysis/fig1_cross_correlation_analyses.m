...
data_dir = 'G:';
sensor_cohort = 1; % set to 1 to run sensor mice, 0 to run control cohort
...
% cohort mice
control_mice = struct;
control_mice.cohort6 = {'mutAD1','mutAD2','mutAD3'};
...
Fc_field = 'Fc';
Fc_artifact_mask = 'artifact_mask';
datafile_suffix = '';
sr = 18; % sampling rate
...
[cc_results,mouse_results] = get_cross_corr_results(mice,fib,data_dir,lag,...
   'null_n',10000,'Fc_field',Fc_field,'Fc_artifact_mask',Fc_artifact_mask,...
   'save_by_mouse',1,'save_dir',save_dir1,'datafile_suffix',datafile_suffix);
...
function [cc_results,mouse_results] = get_cross_corr_results(mice,fib,data_dir,lag,varargin)
    ...
    cc_results = load_if_exist(fullfile(save_dir,'cross_corr_results.mat'));
    mouse_results = load_if_exist(fullfile(save_dir,'cross_corr_mouse_results.mat'));        
    
    
    % initialize output
    if ~isfield(cc_results,'lag')
        cc_results.lag = -lag:lag;
    end
    other_fields_to_check = {'mouse','str_rois','r'};
    for f = 1:numel(other_fields_to_check)
        if ~isfield(cc_results,other_fields_to_check{f})
            cc_results.(other_fields_to_check{f}) = [];
        end
    end
    r_fields_to_check = {'mean','std','n','sig','max','min'};
    for f = 1:numel(r_fields_to_check)
        if ~isfield(cc_results.r,r_fields_to_check{f})
            cc_results.r.(r_fields_to_check{f}) = [];
        end
    end
...
function cc_lags_distr = get_cross_corr_lag_distr(mice,fib,data_dir,lag,...
    ...
    for m = 1:numel(mice)        
        mouse = mice{m};
        mouse_field = get_mouse_field(mouse);
        if ~isfield(cc_lags_distr.min_n,mouse_field)
            disp(['     getting per-mouse min #samples needed: ' mouse])
            % info
...
%         end            
        ...
        for m = 1:numel(mice)        
            ...
            mouse_field = get_mouse_field(mouse);
            disp(mouse)     
            disp(['     compiling cross-correlation results: ' mouse])
            % mouse lag data
            mouse_data = get_mouse_cross_corr_lag_distr(mouse,fib,data_dir,...
                lag,cc_lags_distr.min_n.(mouse_field),'save_dir',save_dir,...
                'Fc_field',Fc_field,'datafile_suffix',datafile_suffix);  
            % mouse cross corr data
            cc_mouse = mouse_results.(mouse_field);
            % add to results
...
function mouse_data = get_mouse_cross_corr_lag_distr(mouse,fib,data_dir,lag,min_n,varargin)
    ...
    % variable specification needed for parfor:
    Fc_field = Fc_field; 
    Fc_artifact_mask = Fc_artifact_mask;
    ...
    if ~isempty(i_pickup) % if we don't have any to do
        while isnan(n(sample_n))
            ...
            parfor i = i_pickup:min([i_pickup+sample_n_chunk sample_n])                                
                ...
                % sometimes for whatever reason these randi lines randomly error (for seemingly no reason)
                max_n = size(DA,1)-2*lag;
                ok_go_ahead = 0;
                while ok_go_ahead == 0 
                    try
                        n_timepoints = randi([min_n max_n]);
                        max_i_start = size(DA,1)-n_timepoints-lag;                    
                        i_start = randi([lag+1 max_i_start]);
                        ok_go_ahead = 1;
                    catch exception
                        disp(['          '  num2str(i)  ' caught exception'])
                    end
                end
    ...
    for s = 1:numel(loc_signs)                               
        ...
        for r = 1:numel(str_rois)    
            ...
            if isempty(mouse_data.lags.(['loc_' loc_signs{s}]).gm_gof{r})
...
function exp_dirs = get_exp_dirs(mouse,data_dir,varargin)
    ...
    for d = 1:numel(exp_dirs)      
        ...
        % if these are control mice and we need to change to ACh and DA fieldnames
        if ~isfield(data,channel_names{1}) || ~isfield(data,channel_names{2}) 
            data = rename_to_ach_da_data_fields(data);
        end
...
function session_cross_corrs = get_session_cross_corrs(mouse,fib,data_dir,lag,varargin)
...
function [gm,gof_vals] = fit_gmm_to_hist(X,varargin)
    ...
    %%%  parse optional inputs %%%
    ip = inputParser;    
    ip.addParameter('gof','BIC');                   % default BIC; other options 'AIC','NegativeLogLikelihood'
    ip.addParameter('gof_vals',[]);                 % in case we already have gof_vals
    ...
    ip.addParameter('reg_val',1e-1);                % regularization value
    ip.parse(varargin{:});
    for j=fields(ip.Results)'
        eval([j{1} '=ip.Results.' j{1} ';']);
    end
    ...
    if isempty(gof_vals)
        ...
        for i = 1:max_n
            gm = fitgmdist(data,i,'Options',options,'Replicates',n_rep,'RegularizationValue',reg_val,'Start','plus');
            gof_vals(i)= gm.(gof);        
        end
    end
    if sum(~isnan(gof_vals))>0
        ...
        gm = fitgmdist(data,n_gauss,'Options',options,'Replicates',n_rep,'RegularizationValue',reg_val,'Start','plus');
    else
        gm = [];
    end
end
...
function k = get_elbow(x, y)
    ...
    % If y is increasing, return the min
    if y(end)>y(1)
        [~,k] = min(y);
        k = x(k);
